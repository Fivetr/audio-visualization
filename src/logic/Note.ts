...
export class Note {  
  ...
  constructor(pitchClass: PitchClass, octave: number, accidental?: AccidentalSymbol){
  ...
  public static noteToFreq = (note: Note | noteProperties) => this.noteIndexToFreq(this.noteToIndex(note));
  static noteIndexToFreq = (noteIndex: number) => 440 * Math.pow(twelthRootOfTwo, noteIndex - 72);
  ...
  static noteToIndex = (note: Note | noteProperties) => (
    ...
    + (note.accidental ? {'bb': -2, 'b': -1, '♮': 0, '#': 1, '##': 2}[note.accidental!] : 0) 
    + 12 * (note.octave + 1)
    + 3
  )
}
