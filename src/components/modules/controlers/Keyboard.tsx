...
const Keyboard = ({
  ...
  return (
    <div className="my-container w-fit gap-2 items-center">
      ...
      <div className="ml-3 flex flex-col gap-2 pt-[2rem]">
        <select
      ...
      <div className="ml-2 flex flex-col gap-2">
        <div>
          <span>
            <Button
            style="freq-button w-10 mr-2 focus:ring-4 focus:outline-none focus:ring-red-300"
            onClick={() => setStartingOctave(Math.max(startingOctave - 1, -2))}
            label="-8ve"
            />
          </span>
          <span>
      ...
      <div className="flex flex-col w-full gap-1">
        {
          Array(numOctaves).fill(null).map((_, index) => (
            <div>
              {
                oneOctaveOfNotes.map(note => (
                  <Key 
                    {...{onHandler, offHandler, setFrequency}} 
                    note={new Note(note.pitchClass, startingOctave + index, note.accidental)}
                  />
                ))
              }
            </div>
